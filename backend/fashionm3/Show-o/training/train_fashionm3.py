...
@torch.no_grad()
def evaluate_fashion_metrics(model, test_dataloaders, uni_prompting, vq_model, accelerator, config, global_step):
    """
    Fashion-specific evaluation on test data
    
    Based on FashionM3 paper Section V.B and Table III metrics:
    - S-BERT Similarity: Semantic similarity of recommendation text
    - CTS (CLIP Text Similarity): Text-image alignment for T2I
    - CIS (CLIP Image Similarity): Generated vs ground-truth images
    - Personalization Score: User preference alignment
    
    Args:
        model: Fine-tuned FashionM3 model
        test_dataloaders: Dict with 'fashionrec' and 't2i' test dataloaders
        uni_prompting: Universal prompting utility
        vq_model: MAGVIT-v2 for image tokenization
        accelerator: Accelerator for distributed evaluation
        config: Training configuration
        global_step: Current training step
    """
    logger.info(f"Evaluating fashion metrics at step {global_step}...")
    model.eval()
    
    eval_metrics = {}
    
    # 1. Evaluate Fashion Recommendation Tasks (MMU flow)
    if 'fashionrec' in test_dataloaders:
        logger.info("Evaluating FashionRec recommendations...")
        fashionrec_metrics = evaluate_recommendation_tasks(
            model, test_dataloaders['fashionrec'], uni_prompting, config
        )
        eval_metrics.update(fashionrec_metrics)
    
    # 2. Evaluate Fashion Image Generation (T2I flow)  
    if 't2i' in test_dataloaders:
        logger.info("Evaluating fashion image generation...")
        t2i_metrics = evaluate_image_generation_tasks(
            model, test_dataloaders['t2i'], uni_prompting, vq_model, config
        )
        eval_metrics.update(t2i_metrics)
    ...
    # 4. Print summary
    logger.info(f"Evaluation Results (Step {global_step}):")
...
def evaluate_recommendation_tasks(model, test_dataloader, uni_prompting, config):
    """
    Evaluate fashion recommendation tasks using S-BERT and Personalization metrics
    Paper Table III: S-BERT=72.69, Personalization score
    """
    # TODO: Implement S-BERT similarity computation
    # TODO: Implement personalization score computation
    
    # Placeholder metrics for now
    return {
        "s_bert_similarity": 0.0,
        "personalization_score": 0.0
    }


def evaluate_image_generation_tasks(model, test_dataloader, uni_prompting, vq_model, config):
    """
    Evaluate fashion image generation using CLIP-based metrics
    Paper Table III: CTS=26.51, CIS=80.37
    """
    # TODO: Implement CLIP text similarity (CTS)
    # TODO: Implement CLIP image similarity (CIS)
    
    # Placeholder metrics for now
    return {
        "clip_text_similarity": 0.0,
        "clip_image_similarity": 0.0
    }
...
def main():
    ...
    if config.dataset.gen_type == "t2i":
    ...
    elif config.dataset.gen_type == "fashion_image_generation":
        ...
        print('process index : ',
              ...
              "Length: ", len(dataset_fashion_img))
        ...
        train_dataloader_t2i = DataLoader(dataset_fashion_img, batch_size=config.training.batch_size_t2i,
                                          sampler=sampler,
    ...
    if config.dataset.und_type == "captioning":
    ...
    elif config.dataset.und_type == "fashionrec":
        # FashionM3 dataset for fashion recommendation tasks
        dataset_fashionrec = FashionRecDataset(
            data_root=dataset_config.fashionrec_data_root,
            split="train",
            task_weights=dataset_config.get("task_weights", None)
        )
        
        if accelerator.num_processes > 1:
            sampler = DistributedSampler(dataset_fashionrec,
                                       num_replicas=accelerator.num_processes,
                                       rank=accelerator.process_index,
                                       shuffle=True)
            shuffle = False
        ...
        train_dataloader_mmu = DataLoader(
            ...
            shuffle=shuffle,
            num_workers=dataset_config.num_workers,
            collate_fn=None  # FashionRecDataset returns dict format directly
        )
    ...
    ##################################
    #     TEST DATALOADERS FOR EVAL  #
    ##################################
    test_dataloaders = {}
    if config.experiment.get("eval_every", None):
        # Test FashionRec dataloader for recommendation evaluation
        test_fashionrec = FashionRecDataset(
            data_root=dataset_config.fashionrec_data_root,
            split="test",
            task_weights=dataset_config.get("task_weights", None)
        )
        test_dataloaders['fashionrec'] = DataLoader(
            test_fashionrec,
        ...
        # Test Fashion Image Generation dataloader for T2I evaluation
        test_fashion_img = FashionImageGenerationDataset(
            data_root=dataset_config.train_t2i_shards_path_or_url,  # Use base path directly
            split="test"
        )
        test_dataloaders['t2i'] = DataLoader(
            test_fashion_img,
            batch_size=config.evaluation.eval_batch_size,
            shuffle=False,  # Don't shuffle test data
            num_workers=dataset_config.num_workers
        ...
        print(f"📊 Test dataloaders created:")
        print(f"  - FashionRec test: {len(test_fashionrec)} samples")
        print(f"  - Fashion Image Generation test: {len(test_fashion_img)} samples")
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch, batch_idx, dataloader_idx in combined_dataloader:
            ...
            if accelerator.sync_gradients:
                ...
                if (global_step + 1) % config.experiment.get("eval_every", 2000) == 0 and accelerator.is_main_process:
                    if test_dataloaders:  # Check if test dataloaders were created and populated
                        evaluate_fashion_metrics(
                            model,
                            test_dataloaders,
                            uni_prompting,
                            vq_model,
